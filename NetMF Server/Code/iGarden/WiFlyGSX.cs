...
namespace Toolbox.NETMF.Hardware
{
    ...
    public class WiFlyGSX : IDisposable
    {
        ...
        private iGarden.WiFlyStream _stream;

        public iGarden.WiFlyStream ListenSocket(int port)
        {
            if (port != 2000)
                throw new ApplicationException("Can only listen on port 2000");

            if (_stream == null)
            {
                this._Mode = Modes.Listening;

                _stream = new iGarden.WiFlyStream(this);
            }
            else
            {
                _stream.WaitForCloseOrData();
            }
        ...
        {
            if (this._SocketOpenString == "" || this._SocketCloseString == "")
        ...
        {
            if ((this._Mode != Modes.StreamingMode) && (this._Mode != Modes.Listening))
                throw new InvalidOperationException("Can't write data when not connected");
        ...
        {
            lock (this._SerialPort_StreamBuffer)
            {
                if (this._SerialPort_StreamBuffer.Length == 0)
                {
                    return "";
                }
            }
        ...
        private void _SerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (this._Mode == Modes.Listening)
            {
                _DebugPrint('L', ReadBuffer);

                lock (this._SerialPort_StreamBuffer)
                {
                    this._SerialPort_StreamBuffer = this._SerialPort_StreamBuffer + ReadBuffer;
                }
